...
const businessComponent = ref(visualConfig.componentModules.businessComponents);

const rows = computed(() => {
  return `grid-rows-${Math.ceil(businessComponent.value.length / 3) + 1}`;
})
...
<template>
  <div
      v-if="businessComponent?.length"
      class="w-full h-full flex"
  >
    <DraggableTansitionGroup
        v-model="businessComponent"
