...
const chartsComponents = ref(visualConfig.componentModules.chartComponents);

const rows = computed(() => {
  return `grid-rows-${Math.ceil(chartsComponents.value.length / 3) + 1}`;
})
...
<template>
  <div
      v-if="chartsComponents?.length"
      class="w-full h-full flex"
  >
    <DraggableTansitionGroup
        v-model="chartsComponents"
